...
x = sv_files[1]
sv.calls = do.call(rbind,lapply(sv_files[1:10],
                                function(x){
                                  # read vcf file
...
sv.calls$ins_seq = gsub('INSERTION=','',str_extract(sv.calls$INFO,'INSERTION=[ACGT]*'))
...
sv.calls$cnt_type = unlist(lapply(grepl('^[AGCT]',sv.calls$ALT),function(x) {ifelse(x,'+','-')}))
