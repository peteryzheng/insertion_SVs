...
kmer_file = 'kmers_1026231833.txt'
# kmer_text <- fread(paste0(workdir, "/youyun/nti/code/outputs/kmers_1026231833.txt"), header = FALSE)
kmer_text <- fread(paste0(intermediate_dir,'/inputs/', kmer_file), header = FALSE)

# jobs number for the task array
################## change this to the task array number you want to use ##################
task_array_number <- 40825613
task_name <- "ins_homeology_1026231833"

# Miscellaneous stuff --------------------------------------------------------------------------------------------------
current_time <- format(Sys.time(), "%m%d%y%H%M")
# current_time <- '1031231405'
...
write.table(task_status_kmer, paste0(intermediate_job_output_dir,'/', task_array_number, "_", current_time, ".tsv"), sep = "\t", quote = FALSE, row.names = FALSE)
print(paste0('Job resource info from UGER can be found here: ', paste0(intermediate_job_output_dir,'/', task_array_number, "_", current_time, ".tsv")))
...
template_task_array <- c(
    "#!/bin/bash",
    "#$ -l h_rt=60:00:00",
    paste0("#$ -t 1-", nrow(fread(paste0(intermediate_dir, "/inputs/failed_kmers_", current_time, ".txt"), header = FALSE))),
    "#$ -pe smp 4 ",
    "#$ -binding linear:4 ",
    "#$ -l h_vmem=4G",
    ...
    "use R-4.0",
    "",
    paste0("kmer=$(sed -n -e \"$SGE_TASK_ID p\"  ", intermediate_dir, "/inputs/failed_kmers_", current_time, ".txt", ")"),
    "echo $kmer",
    paste0(
...
task_array_path <- paste0(intermediate_dir, "/inputs/task_array_rerun_", current_time, ".sh")
