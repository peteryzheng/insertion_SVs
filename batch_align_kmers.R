...
write.table(insertion.sv.calls.subset$ins_seq, commands_text_path, sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
...
template_task_array <- c(
  ...
  "",
  'export PATH="/home/unix/youyun/miniconda3/bin:$PATH"',
  "",
  paste0("kmer=$(sed -n -e \"$SGE_TASK_ID p\"  ", commands_text_path, ")"),
  "echo $kmer",
  paste0(
    "~/miniconda3/bin/conda run -n rameen Rscript /xchip/beroukhimlab/youyun/nti/code/insertion_SVs/align_nearby_utils.R  -i $kmer ",
